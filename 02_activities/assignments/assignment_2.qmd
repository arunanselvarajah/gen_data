...
(i) Read the `.fam` file. How many samples does the dataset contain? 4000
...
(ii) Read the `.bim` file. How many SNPs does the dataset contain? 306,102 SNPs
...
# phenotype is column 6 in .fam
pheno <- fam$V6

ggplot(data.frame(pheno), aes(x = pheno)) +
...
plink2 \
  ...
  --out 02_activities/data/gwa.pruned.A2

plink2 \
  --bfile 02_activities/data/gwa.qc.A2 \
  --extract 02_activities/data/gwa.pruned.A2.prune.in \
  --make-bed \
  --out 02_activities/data/gwa.qc.pruned.A2```
...
```{bash}
plink2 \
  --bfile 02_activities/data/gwa.qc.pruned.A2 \
  --king-cutoff 0.0884 \
  --out 02_activities/data/gwa.king.A2
```
...
plink2 \
  ...
  --extract 02_activities/data/gwa.pruned.A2.prune.in \
  --pca 10 \
  --out 02_activities/data/gwa.pca.A2
```
...
plink2 \
  --bfile 02_activities/data/gwa.qc_unrelated.A2 \
  --covar 02_activities/data/gwa.pca.A2.eigenvec \
  --covar-col-nums 3-5 \
  --glm \
  --out 02_activities/data/gwa.gwas.A2
```
...
gwas <- fread("02_activities/data/gwa.gwas.A2.PHENO1.glm.linear")
...
gwas[, BPcum := BP + cumsum(c(0, head(tapply(BP, CHR, max), -1)))[CHR]]

ggplot(gwas, aes(x = BPcum, y = logP, color = as.factor(CHR))) +
  geom_point(size = 0.6) +
  theme_minimal() +
